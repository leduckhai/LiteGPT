...
class CiderScorer(object):
    ...
    def __init__(self, refs, test=None, n=4, sigma=6.0, doc_frequency=None, ref_len=None):
        ...
        for k in refs.keys():
            # refs {Img_id1:[[ref1, ref2,...], Img_id2:[ref1, ref2,...]]}
            self.crefs.append(cook_refs(refs[k]))
            if test is not None:
                # test {Img_id1: [c1], Img_id2: [c2], ....}
...
def compute_cider(preds, labels):
    cider = Cider()
    gts = {i:[pred] for i, pred in enumerate(preds)}
    res = {i:[label] for i, label in enumerate(labels)}
    cider_score = cider.compute_score(gts=gts, res=res)[0]
    return cider_score